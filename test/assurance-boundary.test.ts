...
test("rejects assurance grant issued in the future", async () => {
  ...
  try {
    ...
    assert.ok(address && typeof address === "object");
    const now = Date.now();
    const response = await fetch("http://127.0.0.1:" + address.port + "/jobs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ task: "summarize_document", maxCostMicrounits: 50000, maxLatencyMs: 3000, minimumQuality: 0.85, privacy: "no-retention", assuranceGrant: { grantId: "grant-future", signature: "test-signature", authorized: true, issuedAt: new Date(now + 60000).toISOString(), expiresAt: new Date(now + 120000).toISOString(), nonce: "nonce-future", task: "summarize_document", providerId: "small-fast-1", maxCostMicrounits: 50000, allowPayment: false, allowTrustedMemoryWrite: false } }) });
    const body = await response.json();
    assert.equal(response.status, 403);
    assert.equal(body.error, "ASSURANCE_EXPIRED_OR_INVALID_TIME");
  } finally {
